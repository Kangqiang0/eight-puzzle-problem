    ...
    function produce(sou, tar) {
        ...
        while ( q.length && ! isSolve ) {
            ...
            for ( var j = 0; j < 4; j++ ) {
             ...
             // 查找完状态，出队列，shift是 js的Array弹队首的方法，pop是给栈用的            
            q.shift();
        }
        ...
        if(!isSolve) return {solve: false, time: endT-beginT, stateNum: count};
         ...
         // 由path记录的路径还原最终路径结果，从source来的，从target来的 都要，还原两次         
        var changePath = [];
